	...
	*((__u32 *)(CR_SPI_BASE | SPI_FLASH_MM)) = reg0x28;
	...
	for (i=0; i < ARRAY_SIZE(flash_tables); i++) {
		if (((MANUFACTURER_ID(flash_id) == flash_tables[i].mfr_id) &&
		    (DEVICE_ID(flash_id) == flash_tables[i].dev_id)) || (strcmp(flash_tables[index].name,"default") == 0)) {
			...
			if(strcmp(flash_tables[i].name,"default") == 0)
			{
				...
				flash_tables[i].dev_id = DEVICE_ID(flash_id);
				flash_tables[i].mfr_id = MANUFACTURER_ID(flash_id);
			}
