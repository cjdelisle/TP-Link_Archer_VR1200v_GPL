...
static int spi_nand_setup(u32 *ptr_rtn_mtd_address)
{
	...
	else
	{
		...
		return -ENOMEM;
				...
				printk("File:%s===>Function:%s Line:%d bmt_pool_size=%d\n", __FILE__, __FUNCTION__, __LINE__, bmt_pool_size);
...
#ifdef TCSUPPORT_NAND_BMT
#define RESERVEAREA_ERASE_SIZE 0x20000
...
#define BOOT_FLAG_OFFSET (MRD_RA_OFFSET - 1) 
#endif
#endif
...
#if defined(CONFIG_PW_MODEL_VMG1312T20AV1)
char *l_p_mtd_name[] = {"bootloader", "romfile", "kernel", "rootfs", "tclinux", "kernel_slave", "rootfs_slave", "tclinux_slave", "wwan", "data", "rom-d", "reservearea", NULL};
#define NP_TOTAL            12
#else
char *l_p_mtd_name[] = {"bootl", "romfile", "kernel", "rootfs", "config", "radio", "wwan", "data", "rom-d", "reservearea", NULL};
#define NP_TOTAL            10
#endif
...
#if defined(CONFIG_PW_MODEL_VMG1312T20AV1)
#define FLASH_IOCTL_READ_NAME		(0x06)
#define FLASH_IOCTL_WRITE_NAME		(0x07)
#endif
...
#if defined(CONFIG_PW_MODEL_VMG1312T20AV1)
extern struct mtd_partition *ecnt_parts;
#else
extern struct mtd_partition tc3162_parts[];
#endif
static struct mtd_info *get_mtd_from_addr(unsigned int *from, unsigned int len)
{
	int i;
	int ret;
	unsigned int mapped = *from;
	unsigned int base;
    unsigned int size;
	...
	{
#if defined(CONFIG_PW_MODEL_VMG1312T20AV1)
		base = ecnt_parts[i].offset;
		size = ecnt_parts[i].size;
#else
		base = tc3162_parts[i].offset;
		size = tc3162_parts[i].size;
#endif
		...
		if (*from >= base && 
            ...
            l_p_mtd_name[i] != NULL)
		{
			 mtd = get_mtd_device_nm(l_p_mtd_name[i]);
			 ...
			 if (!IS_ERR_OR_NULL(mtd))
			 {
				...
				do
		        {
		            if ( (ret = mtd_block_isbad(mtd, mapped)) != 0 )
		            {
		                printk("mtd[%s]: offset %08x is bad\n", mtd->name, mapped);
		                if ((mapped += mtd->erasesize) > size)
		                {
		                    printk("All blocks are bad !\n");
		                    put_mtd_device(mtd);
		        ...
		        printk("mtd[%s]: map address %08x to offset %08x\n", mtd->name, *from, mapped);
		        *from = mapped;
			 ...
			 {
			 	printk("please check mtd device name [%s] !\n", l_p_mtd_name[i]);
				return NULL;
			 }
		}
	}
...
int spiflash_ioctl_read(u_char *buf, u32 from, u32 len)
{
	...
	mtd  = get_mtd_from_addr(&from, len);

	if (mtd == NULL)	{
		printk("%s(): spi nand flash has not init.\n", __FUNCTION__);
		return -EINVAL;
	}
...
{
	int ret = 0;
	unsigned int off_erase;
    unsigned int len_erase;
	unsigned int off_flash;
	struct mtd_info *mtd = NULL;
	...
	if (mtd)
	{
		off_erase = off_flash & (mtd->erasesize - 1);
    	off_flash &= ~(mtd->erasesize - 1);

		do
		{
			len_erase = ((len + off_erase) <= mtd->erasesize) ? len : mtd->erasesize - off_erase;
			
			ret = nandEraseBlk(mtd, off_flash);
			
			off_flash += mtd->erasesize;
			off_erase = 0;
			len -= len_erase;
		} while (len > 0);
...
int ra_mtd_write_nm(char *name, loff_t to, size_t len, const u_char *buf)
{
	...
	block_addr = to & (~(mtd->erasesize - 1));
	addr_in_block = to & (mtd->erasesize - 1);	
	
	if (len > mtd->erasesize - addr_in_block) {
		put_mtd_device(mtd);
		return -E2BIG;
	}
	...
	ret = mtd->read(mtd, block_addr, mtd->erasesize, &rdlen, bak);
	if (ret != 0) {
		put_mtd_device(mtd);
		kfree(bak);
		return ret;
	...
	memcpy(bak + addr_in_block, buf, len);
	...
	ret = mtd->write(mtd, block_addr, mtd->erasesize, &wrlen, bak);
...
int read_mac_from_flash(unsigned char *pMac)
{
	...
	if (spiflash_ioctl_read(pMac, 0x0000FF48, 6) != 0)
	{
		return -1;
	}	
...
long spi_nand_flash_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FLASH_IOCTL_WRITE:
	{
		...
		sector_num = opt->bytes >> 17;
		len = opt->bytes;

		addr = (u32)opt->dest & 0xFFFE0000;					/* first sector addr to erase */
...
#if defined(CONFIG_PW_MODEL_VMG1312T20AV1)
	case FLASH_IOCTL_READ_NAME:
	{
		char mtd_name[MTD_NAME_LEN] = {0};
		copy_from_user(mtd_name, opt->name, MTD_NAME_LEN);

		if (ra_mtd_read_nm(mtd_name, (u32)opt->src, opt->bytes, l_buf) < 0)
		{
			opt->result=OUT_OF_SCOPE;
		}
	...
	case FLASH_IOCTL_WRITE_NAME:
	{
		char mtd_name[MTD_NAME_LEN] = {0};
		copy_from_user(mtd_name, opt->name, MTD_NAME_LEN);
		copy_from_user(l_buf, (unsigned char *)opt->src, opt->bytes);

		if (ra_mtd_write_nm(mtd_name, (u32)opt->dest, opt->bytes, l_buf) < 0)
		{
			opt->result=OUT_OF_SCOPE;
		}
		break;
...
struct file_operations spi_nand_flash_fops = {
    unlocked_ioctl:      spi_nand_flash_ioctl,
};

static int __init spinandflash_chrdev_init(void)
{
	int result=0;
	
	printk(KERN_INFO "Register flash device:flash0\n");
	result = register_chrdev(flash_major, FLASH_DEVNAME, &spi_nand_flash_fops);
	if (result < 0) {
...
static void __exit spinandflash_chrdev_exit(void)
{
	unregister_chrdev(flash_major, FLASH_DEVNAME);
}

module_init(spinandflash_chrdev_init);
module_exit(spinandflash_chrdev_exit);
