...
static struct mtd_info *l_mtd = NULL;
...
unsigned long spinor_spinlock_flags;
...
{	
	/* disable interrupt */
	/*local_irq_disable(); */
	spin_lock_irqsave(&spinorLock, spinor_spinlock_flags);
...
void sf_manual_end(void) 
{
	...
	/* enable interrupt */
	/*local_irq_enable();*/
	spin_unlock_irqrestore(&spinorLock, spinor_spinlock_flags);
}
...
spiflash_read_id (void)
{
	...
	if(down_Manual_interruptible(&SPI_SEM))
		return -ERESTARTSYS;
#else
	down_Normal_interruptible(&SPI_SEM);
#endif
...
static struct spi_flash_info flash_tables[] = {
	...
	/* please note the default configuration must the last one,if add new flash config,
	 * please add before of this,if the using flash do not config at here, we will use 
	 * the defautl configuration to try. DEFAULT_FLASH_SIZE defined at the menuconfig of userspace 
	 * add by panyong 2019-09-18
	 */
	{
		mfr_id: 0,
		dev_id: 0,
		/*please note ,the name cannot be modify, there is some other place to judge it.*/
		name: "default",
		DeviceSize: DEFAULT_FLASH_SIZE,
		EraseSize: SIZE_64KiB,
	},
};
	...
	*((__u32 *)(CR_SPI_BASE | SPI_FLASH_MM)) = reg0x28;
	...
	for (i=0; i < ARRAY_SIZE(flash_tables); i++) {
		if (((MANUFACTURER_ID(flash_id) == flash_tables[i].mfr_id) &&
		    (DEVICE_ID(flash_id) == flash_tables[i].dev_id)) || (strcmp(flash_tables[i].name,"default") == 0)) {
			...
			/* if the flash do not at the support list. we will use the default to try. 
			 * add by panyong 2019-09-18
			 */
			if(strcmp(flash_tables[i].name,"default") == 0)
			{
				printk("the flash not at the support list. we will use defaut configuration to try. please do more test");
			}
...
unsigned char ReadSPIByte(unsigned long index)
{
	...
	spiflash_read(map, index, buf, len);
	kzfree(map);
	return buf[0];
}
...
static int __init spiflash_probe_init(void)
{
	...
	for (index=0; index < ARRAY_SIZE(flash_tables); index++) {
		if (((MANUFACTURER_ID(flash_id) == flash_tables[index].mfr_id) &&
		    (DEVICE_ID(flash_id) == flash_tables[index].dev_id)) || (strcmp(flash_tables[index].name,"default") == 0)) {
			if ( (MANUFACTURER_ID(flash_id) == MANUFACTURER_WINBOND) && (DEVICE_ID(flash_id) == W25Q256) ) {
				is_W25Q256 = 1;
			}
			if ( (MANUFACTURER_ID(flash_id) == MANUFACTURER_MXIC) && (DEVICE_ID(flash_id) == MX25L25635E) ) {
			 ...
			 * add by panyong 2019-09-18
			 */
			if(strcmp(flash_tables[index].name,"default") == 0)
			{
				printk("the flash not at the support list. we will use defaut configuration to try. please do more test");
			}
...
int sf_autobench_check(){//7505???
	...
	/* if the flash do not at the support list. we will use the default to try. it do not need return error
	 * delete panyong 2019-09-18
	 */
#if 0
...
#define VIRT_ADDR	0xbfc00000
#define WINDOW_SIZE 0x400000          
#define BUSWIDTH 	2 

/* also define in tc3162-flash.c */
...
/* defined in tc3162-flash.c */
extern struct map_info tc3162_map;
...
int spiflash_ioctl_read(u_char *buf, u32 from, u32 len)
{
	...
	if (l_mtd == NULL)	{
		printk("%s(): spi flash has not init.\n", __FUNCTION__);
		return -EINVAL;
	}

	return mtd_spiflash_read(l_mtd, from, len, &retlen, buf);
}
...
int spiflash_ioctl_write(u_char *buf, u32 to, u32 len)
{
	...
	printk("Write to flash:0x%x, len is 0x%x\n", to, len);

	return mtd_spiflash_write(l_mtd, to, len, &retlen, buf);
}
...
int spiflash_ioctl_erase(u32 to, u32 len)
{
	...
	int ret = 0;
	struct spi_chip_info *chip_info = (struct spi_chip_info *)tc3162_map.fldrv_priv;
    ...
    printk("Erase from 0x%x with 0x%x bytes, mtd->erasesize 0x%x\n", to, len, l_mtd->erasesize);
	...
	*((__u32 *)(CR_SPI_BASE | SPI_FLASH_MM)) = reg0x28;
	...
	if (to & (chip_info->flash->EraseSize - 1)){
#if defined(TC_SOC) && defined(CONFIG_MIPS_TC3262)
		up_Manual(&SPI_SEM);
#endif
#if defined(TCSUPPORT_VOIP)
	...
	if ((len + to) > chip_info->flash->DeviceSize){
#if defined(TC_SOC) && defined(CONFIG_MIPS_TC3262)
		up_Manual(&SPI_SEM);
#endif
#if defined(TCSUPPORT_VOIP)
	...
	do {
#if 0	/* dbs: delete for avoid kernel panic 22Mar16 */
		if (!spiflash_wait_ready(FL_ERASING)){
#if defined(TC_SOC) && defined(CONFIG_MIPS_TC3262)
			up_Manual(&SPI_SEM);
#endif
...
#endif /* #if 0 */
		ret = spiflash_erase(l_mtd, addr_start); /* dbs: change parameter 22Mar16 */
//		spiflash_done(); /* dbs: delete for avoid kernel panic 22Mar16 */
		if (ret){
#if defined(TC_SOC) && defined(CONFIG_MIPS_TC3262)
			up_Manual(&SPI_SEM);
#endif
		...
		addr_start += (chip_info->flash->EraseSize);
		printk(".");
		/*clear watchdog counter when update firmware */
#ifdef TCSUPPORT_WATCHDOG
		tc3162wdog_kick();
#endif /* TCSUPPORT_WATCHDOG */
...
#if defined(CONFIG_PW_HW_FLASH_SIZE_8M)
#define CONFIG_DATA_FLASH_START  0x007E0000
#elif defined(CONFIG_PW_HW_FLASH_SIZE_16M)
#define CONFIG_DATA_FLASH_START  0x00FE0000
#endif
...
void do_factory_reset(void)
{
	u8 buff[128] = {0};
	local_irq_disable();
	spiflash_ioctl_erase(CONFIG_DATA_FLASH_START, 0x10000);
	spiflash_ioctl_write(buff, CONFIG_DATA_FLASH_START, 0x10000);
	machine_restart(NULL);
}
EXPORT_SYMBOL(do_factory_reset);
...
long flash_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FLASH_IOCTL_WRITE:
	{
		...
		tc3162wdog_kick();
		timerWdSet(ENABLE, ENABLE);
		
		printk("TC3162 hardware watchdog restart\n");
#endif
		break;
    }	
...
#if defined(CONFIG_PW_HW_FLASH_SIZE_16M)
	if (spiflash_ioctl_read(pMac, 0x00FDF100, 6) != 0)
	{
		return -1;
	}
#endif
...
/* Add by Zhao Mengqing, 2016-10-10, To see if DUT is Caled */
#if defined(CONFIG_PW_HW_FLASH_SIZE_8M)
#define RADIO_2G_FLASH_START  0x007F0000
#define RADIO_5G_FLASH_START  0x007F8000
#elif defined(CONFIG_PW_HW_FLASH_SIZE_16M)
#define RADIO_2G_FLASH_START  0x00FF0000
#define RADIO_5G_FLASH_START  0x00FF8000
#endif
int is_dut_caled(void)
{	
	unsigned char calData_2g[2], calData_5g[2];

#if defined(CONFIG_PW_HW_PVW41T300N_DESIGN)
	if (spiflash_ioctl_read(calData_2g, RADIO_2G_FLASH_START, 2) != 0)
	{
		return -1;
	}
	if ((0xff == calData_2g[0] && 0xff == calData_2g[1]) 
		|| (0x0 == calData_2g[0] && 0x0 == calData_2g[1]))
	{
		return 0;
	}
	else
	...
	if (spiflash_ioctl_read(calData_5g, RADIO_5G_FLASH_START, 2) != 0)
	{
		return -1;
	}
	if (((0xff == calData_2g[0] && 0xff == calData_2g[1]) 
		  || (0x0 == calData_2g[0] && 0x0 == calData_2g[1]))
			|| ((0xff == calData_5g[0] && 0xff == calData_5g[1]) 
				|| (0x0 == calData_5g[0] && 0x0 == calData_5g[1])))
	{
		return 0;
	}
	else
