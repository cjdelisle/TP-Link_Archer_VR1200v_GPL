...
/* Zhang Wenkai, 2016/7/15. */
#include <linux/etherdevice.h>  
extern int read_mac_from_flash(unsigned char *pMac);
...
static struct ipv6_devconf ipv6_devconf __read_mostly = {
	...
	.slaac_addr		="",
	.if_flags		= 0,			/* Add by CM, 2014-06-10 */
#endif
};
...
/* br0 HW address, Zhang Wenkai, 2016/7/15. */
static u8 br_hw_addr[ETH_ALEN] = {0};
...
static int addrconf_ifid_eui48(u8 *eui, struct net_device *dev)
{
	if (dev->addr_len != ETH_ALEN)
		return -1;
	/* Changed by Zhang Wenkai, 2016/7/15.
     * for br0 link-local address with setting HW address other than random HW address generated while br_dev_setup.
	 */
	 if ((strstr(dev->name, "br") != NULL) && is_valid_ether_addr(br_hw_addr))
	 {
		 printk ("use br_hw_addr!\n");
		 memcpy(eui, br_hw_addr, 3);
		 memcpy(eui + 5, br_hw_addr + 3, 3);
	 }
	 else
	 {
...
static void addrconf_dev_config(struct net_device *dev)
{
	...
	if(is_wan_dev(dev) || (strstr(dev->name, "br") != NULL)){
		...
		/* Changed By Zhang Wenkai, 2016/7/14, for BUG 130879. */
#if 0   
...
static struct addrconf_sysctl_table
{
	...
	.addrconf_vars = {
		...
		{							/* Add by CM, 2014-06-10 */
			.procname	=	"if_flags",
			.data		=	&ipv6_devconf.if_flags,
			.maxlen		=	sizeof(int),
			.mode		=	0644,
			.proc_handler	= proc_dointvec
		},
...
int __init addrconf_init(void)
{
	...
	/* read MAC from flash. */
	read_mac_from_flash(br_hw_addr);
