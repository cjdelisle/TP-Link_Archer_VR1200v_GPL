...
int igmp_hwnat_get_port(struct net_bridge_port*  p)
{
	...
	if ((p->dev->name[0] == 'e') && (p->dev->name[4] == '.')) {
			...
			if (g_port_reverse_kernel == 1 && (p->dev->name[5] - '2') <= 3)
				return  3 - (p->dev->name[5] - '2');
			else
			return  (p->dev->name[5] - '2');
#endif
	}
#ifdef TCSUPPORT_MULTICAST_SPEED
...
int igmp_hwnat_port_mask(struct net_bridge *br,IGMP_HWNATEntry_t* entry)
{
    ...
    int switch_port = 0;
...
#if 1 /* extend to support multi bridge, by hqj 2017.12.11*/
         ...
         for_each_netdev(net, dev) {
                   ...
                   {
                            struct net_bridge *b=(struct net_bridge *)netdev_priv(dev);

                            list_for_each_entry(p, &b->port_list, list)
                            {
                                     if (igmp_hwnat_should_deliver(b,p,entry)==0)
                                     continue; 
