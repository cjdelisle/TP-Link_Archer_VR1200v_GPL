 ...
 *         \arg 1.1, 25Nov11, zhulu, modified some function interfaces for kernel 2.6.30.9
 		   \arg 1.2, 6Jul17, zhangwenkai, adapt to IPv6
 *
...
/*add for wan block(also defined in wportal_core.c) by frl,20151225*/
#define SOFT_PPA_SPEED 0x20000000 
/* End adding by fang ruilong*/
/**************************************************************************************************/
/*                                           TYPES                                                */
/**************************************************************************************************/
struct hash_url {
...
static inline struct tcphdr *tp_tcp_hdr(const struct sk_buff *skb)
{
	...
	{
		return (void *)ip_hdr(skb) + ip_hdr(skb)->ihl * 4;;
	}
	else if (ntohs(skb->protocol) == ETH_P_IPV6 && ipv6_hdr(skb)->version == 6)
	{
		...
		tcphoff = ipv6_skip_exthdr(skb, sizeof(struct ipv6hdr), &nexthdr);
...
static inline struct udphdr *tp_udp_hdr(const struct sk_buff *skb)
{
	...
	else if (ntohs(skb->protocol) == ETH_P_IPV6 && ipv6_hdr(skb)->version == 6)
	{
		...
		udphoff = ipv6_skip_exthdr(skb, sizeof(struct ipv6hdr), &nexthdr);
...
static bool match(const struct sk_buff *skb, const struct xt_match_param *par)
{
	...
	if (info->type == MASSURL_TYPE_HTTP || info->type == MASSURL_TYPE_URL)
	{
		...
		{
			/*add for wan block(wportal.ko)by frl,201525*/
			ct->mark |= SOFT_PPA_SPEED;
