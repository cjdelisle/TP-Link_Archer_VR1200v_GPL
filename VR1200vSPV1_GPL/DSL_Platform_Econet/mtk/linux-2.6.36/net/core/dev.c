...
/* brief ???¡§¡ä¡§?D¡§¡è???¨¢?WAN|¨¬?¡§¡é??¡§¡é??¡§¡ä¡§|¡§¡§??¡§oy?Y??¡§1|¨¬?mark?¨¢???|¨¬?|¨¬¡§215???a1??¨º	zl added 2012-3-17 */
...
static int __netif_receive_skb(struct sk_buff *skb)
{
	...
	if(skb->protocol != cpu_to_be16(ETH_P_8021Q)
		&& skb->dev && ((skb->dev->priv_flags & IFF_802_1Q_DUAL_VLAN) == 0) && 
		  (skb->dev->priv_flags & IFF_WAN_DEV))
	{
		struct vlan_ethhdr *veth = (struct vlan_ethhdr *)skb->data;
		if(skb->data - skb->head  < sizeof(*veth))
		{
			if (skb_cow_head(skb, sizeof(*veth)) < 0) 
			{
				kfree_skb(skb);
		    		return NET_RX_DROP;
			}
		...
		/* now, the TCI */
		veth->h_vlan_TCI = htons(0);
		skb->protocol = htons(ETH_P_8021Q);
		skb_pull(skb, sizeof(*veth) - VLAN_HLEN);		
	}
...
/* xcl add, 20130315, for single pvc multi-connect */
/**
 *	init_extnetdev_mq - init network device for ext netdev(Wenzhenyu@TP-LINK Porting from alloc_netdev_mq(), 27Apr12)
 *	@dev:		ext netdev
 *	@name:		device name format string
 *	@setup:		callback to initialize device
 *	@queue_count:	the number of subqueues to allocate
 *
 *	Performs basic initialization for ext netdev.  Also allocates subquue structs
 *	for each queue on the device at the end of the netdevice.
 */
struct net_device *init_extnetdev_mq(struct net_device *dev, void (*setup)(struct net_device *), 
...
EXPORT_SYMBOL(init_extnetdev_mq);
/* end add by xcl */
...
/**
 *	free_extnetdev - free network device for ext netdev(Wenzhenyu@TP-LINK Porting from alloc_netdev_mq(), 27Apr12)
 *	@dev: device
 *
 *	This function does the last stage of destroying an allocated device
 * 	interface. The reference to the device object is released.
 *	If this is the last reference then it will be freed.
 */
void free_extnetdev(struct net_device *dev)
{
	struct napi_struct *p, *n;
