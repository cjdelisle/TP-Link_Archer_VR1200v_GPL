...
#define WLAN_IFDOWN_COS "WLAN REBOOT"
#define WLAN_WPS_COS_5G "WLAN 5G WPS"
#define WLAN_PINLOCK_COS "WSC PIN LOCK"
#define WLAN_PINLOCK_COS_5G "WSC 5G PIN LOCK"
#define WLAN_24G_ITF_NAME "ra0"
#define WLAN_5G_ITF_NAME "rai0"
/**************************************************************************************************/
/*                                           TYPES                                                */
/**************************************************************************************************/
...
static void sendWlanPinLockToCos(char* type)
{
	CMSG_FD msgFd;
	CMSG_BUFF msgBuff;
	...
	if (0 == memcmp(type, WLAN_PINLOCK_COS, strlen(WLAN_PINLOCK_COS)))
	{
		strncpy(msgBuff.content, WLAN_24G_ITF_NAME, strlen(WLAN_24G_ITF_NAME));
	} 
	else if (0 == memcmp(type, WLAN_PINLOCK_COS_5G, strlen(WLAN_PINLOCK_COS_5G)))
	{
		strncpy(msgBuff.content, WLAN_5G_ITF_NAME, strlen(WLAN_5G_ITF_NAME));
	}
	msgBuff.type = CMSG_WPS_PIN_SWITCH;
	msg_init(&msgFd);
	msg_connSrv(CMSG_ID_COS, &msgFd);
...
static void proc_wireless_event(int ifindex, char *data, int len)
{
	...
	else if ((0 == memcmp(data, WLAN_PINLOCK_COS, strlen(WLAN_PINLOCK_COS)))
		|| (0 == memcmp(data, WLAN_PINLOCK_COS_5G, strlen(WLAN_PINLOCK_COS_5G))))
	{
		sendWlanPinLockToCos(data);
		return;
	}
	...
	else if (0 == memcmp(data, WLAN_IFDOWN_COS, strlen(WLAN_IFDOWN_COS)))
	{
		//Shutdown WIFI interface via shutdown_all_wifi()/halt_reboot_pwoff() in busybox
		system("reboot");
	}
