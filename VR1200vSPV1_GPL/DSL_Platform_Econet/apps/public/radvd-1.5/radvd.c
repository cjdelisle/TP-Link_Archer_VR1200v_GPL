...
/* 
 * fn		int check_prefix_delegated(void)
 * brief	Before we send RA, check whether delegated prefix exists.
 * details	When we choose prefix delegated, and router haven't got this prefix from WAN, 
 *			it means that this router should not be default route.
 *			the Router Lifetime of RA should be zero.
 *			
 *			But if we choose Static Prefix, and set one prefix manually, WAN side possibly
 *			have no valid IPv6 connection, then Router Lifetime of RA will not be zero, it 
 *			may have some problem.
 *			
 * param[in]	N/A
 * param[out]	N/A
 *
 * return	0 - prefix exists.
 * retval	-1 - prefix doesn't exist.
 *
 * note		fix bug 84940.
 */
int check_prefix_delegated(void)
{
	struct Interface *iface;
	...
	for (iface=IfaceList; iface; iface=iface->next) {
		...
		{
			print_addr(&(iface->AdvPrefixList->Prefix), addrStr);
			flog(LOG_DEBUG, "Delegated prefix exists, it is: %s/%u.",
				addrStr, iface->AdvPrefixList->PrefixLen);
			return 0;
		}
	}
