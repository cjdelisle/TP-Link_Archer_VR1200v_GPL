...
#include <pathnames.h>
/* 
 * fn		static int check_ip6_dftroute(void)
 * brief	Before we send RA, check whether IPv6 default route exists.
 *
 * details	By default, an IPv6 CE router that has no default router(s) on
 ...
 *
 * return	int
 * retval	1 - ipv6 default route exists
 *			0 - not found 
 *			-1 - open file errors
 *
 * note		[RFC7084] G-4
 */
static int check_ip6_dftroute(void)
{
	FILE *fp = NULL;
	char intfaceName[16] = {0};
	char dstAddr[40] = {0};
	char dftGwAddr[40] = {0};
	unsigned long int plen = 0;

	fp = fopen(PATH_PROC_NET_IPV6_ROUTE, "r");
	if (NULL == fp) {
		flog(LOG_ERR, "Cannot open %s.", PATH_PROC_NET_IPV6_ROUTE);
		return (-1);
	}

	while (4 == fscanf(fp, "%s%lx%*s%*s%s%*s%*s%*s%*s%s", dstAddr, &plen, dftGwAddr, intfaceName))
	{
		if ((0 == strcmp(dstAddr, IPV6_ROUTE_ANY_ADDRESS)) &&
			(0 == plen) &&
			(0 != strcmp(intfaceName, "lo")))
		{
			flog(LOG_DEBUG, "Found ipv6 default route %s gw %s", dstAddr, dftGwAddr);
			fclose(fp);
			return 1;
		}
	}
...
send_ra(int sock, struct Interface *iface, struct in6_addr *dest)
{
	...
	/* 去除前缀检查，改为检查默认路由 changed by ZFN 2016_10_26 */
	radvert->nd_ra_router_lifetime	 =  
		(!check_ip6_forwarding() && check_ip6_dftroute()) ? htons(iface->AdvDefaultLifetime) : 0;
	radvert->nd_ra_flags_reserved   |=
