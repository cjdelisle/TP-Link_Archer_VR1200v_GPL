				 ...
				 pj_dns_parsed_packet *response)
{
	...
	for (j=0; j<query_job->srv_cnt; ++j) {
		...
		/* modified by yuanjp, 2015/11/24 fix memory overflow bug */
		if (cnt < ADDR_MAX_COUNT)
		{
...
//lizhanbo find next dns srv
pj_status_t pj_dns_resolver_srv_next(void* obj)
{
	pj_status_t status = PJ_SUCCESS;
	pj_dns_srv_async_query* query_job = (pj_dns_srv_async_query*)obj;
	...
	tmpFd = fopen("/var/tmp/sip_dnssrv", "a+");
	fgets(line, sizeof(line), tmpFd);
	...
	do{	
		status = resolve_hostnames(query_job);
		PJ_LOG(4,(query_job->objname, 
					"resolve_hostnames=%d(%s,%d)", status, __FUNCTION__, __LINE__));
	}while(status != PJ_SUCCESS && status != PJ_EDNSSRVNONE);
