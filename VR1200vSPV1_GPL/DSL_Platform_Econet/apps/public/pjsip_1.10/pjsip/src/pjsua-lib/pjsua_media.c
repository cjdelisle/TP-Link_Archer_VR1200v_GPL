...
#ifdef INCLUDE_BROADSOFT
	pj_uint8_t priority = PJMEDIA_CODEC_PRIO_DISABLED;
#endif
#	if defined(SUPPORT_IPCALL_NO_ACCOUNT) && SUPPORT_IPCALL_NO_ACCOUNT!=0
	if (call->acc_id>=0)
	{
...
#	if defined(INCLUDE_PSTN_GATEWAY) || \
		...
		/* switch to lite codec mgr, which only contains G.711 codec(PCMU & PCMA), it will be used to create SDP in fax */
	 	pjsua_codec_mgr_switch_to_lite();
#ifdef INCLUDE_BROADSOFT
		priority = PJMEDIA_CODEC_PRIO_NORMAL;
#endif
#	endif
	 }
	 else
	...
	/* 
	 * brief	when dial via IP address, acc will be NULL and no codec will be added to SDP
	 * added by meizaihong, 2017-04-07
	 */
#ifdef INCLUDE_BROADSOFT
	if (!acc)
	{
	 ...
	 for(i = 0; i < count; ++i)
	 {
	 	codec = &c[i].codec_id;
		/* 
		 * Initialize prio of all codecs to PJMEDIA_CODEC_PRIO_DISABLED, 
		 * i.e. disable all the codecs by default. prio of codecs will be 
		 * set according to the configuration. The SDP will only contains 
		 * codecs in the configuration, not all codecs we support.
		 * for broadsoft certification, modified by meizaihong, 2017-02-21
		 */
#ifndef INCLUDE_BROADSOFT
		status = pjsua_codec_set_priority(codec, PJMEDIA_CODEC_PRIO_NORMAL);
#else
		/* when use lite codec mgr, enable all codecs, otherwise, disable them all first */
		status = pjsua_codec_set_priority(codec, priority);
#endif /* INCLUDE_BROADSOFT */
...
#	endif
	...
	/* 
	 * when there is no remote SDP, it means we create the initial SDP and let the remote negotiate
	 * modified by meizaihong, 2017-02-21
	 */
#ifdef INCLUDE_BROADSOFT
	if (!rem_sdp)
	{
