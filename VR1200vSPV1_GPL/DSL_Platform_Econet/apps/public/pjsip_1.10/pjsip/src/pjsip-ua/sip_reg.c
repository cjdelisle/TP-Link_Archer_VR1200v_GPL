				       ...
				       pjsip_regc **p_regc)
{
	...
	if (status != PJ_SUCCESS)
	{
		/*destroy the locak and release the pool. By YuChuwei*/
...
#	if defined(SUPPORT_ACCOUNT_RTT) && SUPPORT_ACCOUNT_RTT!=0
	memset(&regc->regStart, 0, sizeof(regc->regStart));
	memset(&regc->regFirstRps, 0, sizeof(regc->regFirstRps));
#	endif
...
#	if defined(SUPPORT_ACCOUNT_RTT) && SUPPORT_ACCOUNT_RTT!=0
			  , struct timeval start, struct timeval firstRps
#	endif
			  )
{
...
#	if defined(SUPPORT_ACCOUNT_RTT) && SUPPORT_ACCOUNT_RTT!=0
		, regc->regStart, regc->regFirstRps
#	endif
		);
...
#	if defined(SUPPORT_ACCOUNT_RTT) && SUPPORT_ACCOUNT_RTT!=0
	 /*ycw-pjsip.get the time when recv the first response.*/
	 if (0 == regc->regFirstRps.tv_sec && 0 == regc->regFirstRps.tv_usec)
	 {
	 	gettimeofday(&regc->regFirstRps, NULL);	
	 }
#	endif
				...
				tsx->status_code == PJSIP_SC_UNAUTHORIZED)
    {
		...
		status = pjsip_auth_clt_reinit_req(&regc->auth_sess, rdata, tsx->last_tx, &tdata);
		if (status == PJ_SUCCESS)
		{
		    status = pjsip_regc_send(regc, tdata);
#ifdef INCLUDE_TFC_ES
			regc->authState = ACC_AUTH_DOING;
#endif
		}
...
#	endif
	...
	cseq_hdr = (pjsip_cseq_hdr*)pjsip_msg_find_hdr(tdata->msg, PJSIP_H_CSEQ, NULL);
	cseq_hdr->cseq = cseq;

	/* Find Expires header */
	expires_hdr = (pjsip_expires_hdr*)pjsip_msg_find_hdr(tdata->msg, PJSIP_H_EXPIRES, NULL);
...
#	if defined(SUPPORT_ACCOUNT_RTT) && SUPPORT_ACCOUNT_RTT!=0
	/*ycw-pjsip. get the time to send REGISTER request*/
	if (PJ_SUCCESS == status)
	{
	 	gettimeofday(&regc->regStart, NULL);
	}
#	endif
