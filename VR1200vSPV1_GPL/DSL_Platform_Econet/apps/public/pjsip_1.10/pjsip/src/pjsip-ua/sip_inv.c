						...
						pjsip_event *e)
{
	 ...
	 */
	{
	    ...
	    /* dingcheng -pjsip: clear old remote info*/
	    inv->dlg->remote.info->tag.ptr = NULL;
	    inv->dlg->remote.info->tag.slen = 0;
					 ...
					 pjsip_event *e)
{
	...
	((tsx->status_code == PJSIP_SC_CALL_TSX_DOES_NOT_EXIST &&
	 ...
	 tsx->status_code == PJSIP_SC_TSX_TRANSPORT_ERROR*/))/* bugfix#1619 trac.pjsip.org */
    {
		pjsip_tx_data *bye;
		pj_status_t status;
...
CMSIP_PRINT("==========process call========================");
	...
	/* 
	 * when current state is EARLY and receive 18x, we should inform voip_client the status now
	 * added by meizaihong, 2016-03-13
	 */
...
CMSIP_PRINT("==========process call========================");
   ...
   PJ_ASSERT_ON_FAIL(tsx && dlg, return);
	...
	if (tsx->state == PJSIP_TSX_STATE_TRYING) {
		...
		/* bugfix#1291 SDP negotiator state should be reverted back after an SDP re-offer is rejected by application. */
