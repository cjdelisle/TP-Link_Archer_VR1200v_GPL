					    ...
					    pjsip_host_info *dest_info )
{
	...
	*/
    	if (topmost_route_uri)
	{
		...
		{
			const pjsip_sip_uri *url = (const pjsip_sip_uri*)pjsip_uri_get_uri((const void*)topmost_route_uri);
		...
		{
	    		new_request_uri = (const pjsip_uri*)pjsip_uri_get_uri((pjsip_uri*)topmost_route_uri);
			pj_list_erase(first_route_hdr);
	...
	*/
	if (new_request_uri && new_request_uri!=tdata->msg->line.req.uri)
	{
		...
		route->name_addr.uri = (pjsip_uri*)pjsip_uri_get_uri(tdata->msg->line.req.uri);
		if (last_route_hdr)
					 ...
					 pj_ssize_t sent )
{
	...
	for (;;)
	{
		...
		else
		{
			...
			*/
			cont = (sent > 0) ? PJ_FALSE : (tdata->dest_info.cur_addr<tdata->dest_info.addr.count-1);
...
stateless_send_resolver_callback( pj_status_t status,
				  void *token,
				  const struct pjsip_server_addresses *addr,void *other)
{
	pjsip_send_state *stateless_data = (pjsip_send_state*) token;
	pjsip_tx_data *tdata = stateless_data->tdata;

	/*lizhanbo-dns return dns srv none*/
	if (PJ_EDNSSRVNEXT == status)
	{
		PJ_LOG(4,(THIS_FILE, "get status PJ_EDNSSRVNEXT(%s,%d)", __FUNCTION__, __LINE__));
		tdata->dns_obj = other;
		return PJ_EDNSSRVNONE;
	}
				   ...
				   pjsip_send_callback cb)
{
	...
	*/
	if (tdata->dummy_dest_info.name.slen &&
		!pj_strcmp(&tdata->dummy_dest_info.name, &dest_info.addr.host))
	{
		CMSIP_PRINT("--The target uri of this time is the same as the INVITE's target uri"
			", so use the same ip---------------");
		memcpy(&tdata->dest_info, &tdata->dummy_dest_info, sizeof(tdata->dest_info));
	}

	if (tdata->dest_info.addr.count == 0)
	{
		PJ_LOG(4, (THIS_FILE, "==resolve target=="));
	...
	{
		PJ_LOG(4,(THIS_FILE, "%s: skipping target resolution because "
	                     "address is already set",
			     pjsip_tx_data_get_info(tdata)));
		stateless_send_resolver_callback(PJ_SUCCESS, stateless_data,
					 &tdata->dest_info.addr, NULL);
	}
