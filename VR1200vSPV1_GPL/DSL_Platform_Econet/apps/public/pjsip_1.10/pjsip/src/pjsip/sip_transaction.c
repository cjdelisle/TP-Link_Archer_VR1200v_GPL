					  ...
					  pjsip_transaction **p_tsx)
{
	...
	PJ_ASSERT_RETURN(tdata->msg->line.req.method.id != PJSIP_ACK_METHOD, PJ_EINVALIDOP);
...
#ifdef PRECALC_HASH
	tsx->hashed_key = pj_hash_calc(0, tsx->transaction_key.ptr, tsx->transaction_key.slen);
#endif

	PJ_LOG(6, (tsx->obj_name, "tsx_key=%.*s", tsx->transaction_key.slen, tsx->transaction_key.ptr));
	...
	*/
	...
	PJ_LOG(5,(tsx->obj_name, "Transaction created for %s", pjsip_tx_data_get_info(tdata)));
...
static pj_status_t tsx_send_msg( pjsip_transaction *tsx, pjsip_tx_data *tdata)
{
	...
	*/
	if (tsx->transport)
	{
		CMSIP_PRINT("-----tsx has transport-remote(%.*s:%d)----------------",
			tsx->transport->remote_name.host.slen, tsx->transport->remote_name.host.ptr,
			tsx->transport->remote_name.port);
		status = pjsip_transport_send( tsx->transport, tdata, &tsx->addr, tsx->addr_len, tsx, 
				       &transport_callback);
	...
	*/
	...
	if (tdata->msg->type == PJSIP_REQUEST_MSG)
	{
		...
		/*ycw-pjsip-20120612*/
		if (tsx->server_name.slen)
		{
			pj_strdup( tdata->pool, &tdata->dummy_dest_info.name, &tsx->server_name);
			memcpy(&tdata->dummy_dest_info.addr, &tsx->server_addr, sizeof(tsx->server_addr));
			tdata->dummy_dest_info.cur_addr = tsx->cur_addr;
		}
		
		status = pjsip_endpt_send_request_stateless(tsx->endpt, tdata, tsx, &send_msg_callback);
		if (status == PJ_EPENDING)
	    	{
	    		status = PJ_SUCCESS;
		}
...
static pj_status_t tsx_retransmit( pjsip_transaction *tsx, int resched)
{
	...
	//lizhanbo retransmit dns svr query
	if ((PJSIP_REGISTER_METHOD == tsx->method.id && 10 == tsx->retransmit_count) 
	   	&& PJSIP_ROLE_UAC == tsx->role && 0 == tsx->status_code && PJ_FALSE == tsx->updateTransport && getDNSSRV())
	{
		pjsip_tx_data* tdata = tsx->last_tx;

		PJ_LOG(4,(THIS_FILE, "now, we have resent 11 times, should switch to next SIP URL(%s,%d)", __FUNCTION__, __LINE__));		

		tsx->updateTransport = PJ_TRUE;		
		tsx->transport_flag &= ~(TSX_HAS_PENDING_RESCHED);
		
		status = pj_dns_resolver_srv_next(tdata->dns_obj);		
		if (PJ_SUCCESS != status)
		{
			PJ_LOG(4,(THIS_FILE, "fail to switch to next SIP URL. terminate register.status=%d(%s,%d)", status, __FUNCTION__, __LINE__));
			if (tsx->timeout_timer.id != 0)
			{
				pjsip_endpt_cancel_timer(tsx->endpt, &tsx->timeout_timer);
				tsx->timeout_timer.id = 0;
			...
			tsx->transport_flag &= ~(TSX_HAS_PENDING_RESCHED);
			tsx_set_status_code(tsx, PJSIP_SC_TSX_TRANSPORT_ERROR, NULL);
			tsx_set_state(tsx, PJSIP_TSX_STATE_TERMINATED, PJSIP_EVENT_UNKNOWN, NULL);
			return status;
		}
		return PJ_SUCCESS;
	}
...
static pj_status_t tsx_on_state_null( pjsip_transaction *tsx, 
                                      pjsip_event *event )
{
	CMSIP_PRINT("==========process transaction========================");
	pj_status_t status;
 ...
 * is received.
 */
static pj_status_t tsx_on_state_calling( pjsip_transaction *tsx, pjsip_event *event )
{
CMSIP_PRINT("==========process transaction========================");
	       ...
	       event->body.timer.entry == &tsx->timeout_timer) 
    {
		...
		/*ycw-pjsip.*/
		#	if 0
		if ( PJSIP_ROLE_UAC == tsx->role &&
				PJSIP_INVITE_METHOD == tsx->method.id &&
				0 == tsx->status_code)
		{
			tsx->send100ToCm = PJ_TRUE;
		}
		#	endif
	 ...
	 else if (event->type == PJSIP_EVENT_RX_MSG)
	 {
		...
		if (msg->type != PJSIP_RESPONSE_MSG)
	   {
		...
		if (tsx->method.id==PJSIP_OPTIONS_METHOD && tsx->role==PJSIP_ROLE_UAC 
				&& (-1==tsx->accId || tsx->accId>=0))
		{
			cmsip_send_genres(tsx->accId, "OPTIONS", code);
		}
...
CMSIP_PRINT("==========process transaction========================");
	...
	/* Inform TU. */
	tsx_set_state( tsx, PJSIP_TSX_STATE_COMPLETED, PJSIP_EVENT_RX_MSG, event->body.rx_msg.rdata);
