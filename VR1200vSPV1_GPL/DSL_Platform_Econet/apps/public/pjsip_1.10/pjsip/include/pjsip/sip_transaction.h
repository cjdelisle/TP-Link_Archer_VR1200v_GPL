...
struct pjsip_transaction
{
	...
	*/
	...
	pj_bool_t			updateTransport; /*lizhanbo-dns If true, current SIP URL is error,switch to next.*/
	...
	#	if 0
	pj_bool_t	send100ToCm; /*ycw-pjsip. If transaction send INVITE request and 
	 				then timeout, not receive Response 100, this member is true. */
	#	endif

	/*ycw-pjsip-20120611
	If the target uri of the initial INVITE request is a domain name, I save it and its ip here.
	And if a subsequent request has the same target domain name, use this ip.
	*/
