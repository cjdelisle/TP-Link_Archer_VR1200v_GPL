...
/* 这里使用_remoteAddr是为了后续可能使用msg_send的情况(msg_sendAndGetReplyWithTimeout()) */
{
	...
	/* Changed by ZhangWenkai, 2016.05.26, for bugs may occur while receiving without select. */
	while (-1 == recvfrom(pMsgFd->fd, pMsgBuff, MSG_SIZE, 0,
...
int msg_sendAndGetReplyWithTimeout(CMSG_FD *pMsgFd, CMSG_BUFF *pMsgBuff, int timeSeconds)
{
	...
	/* Changed by ZhangWenkai, 2016.05.26. it's safe for implementing as msg_send. */
	}while((-1 == ret) && ((EINTR == errno) || (ENOENT == errno)));
	...
	/* changed by ZhangWenkai, 2016.05.25, for invalid timeout.
	   I don't set a max try times because mostly messages can be
	   done once, only few recvfrom will be interrupted by system. */
	while (-1 == recvfrom(pMsgFd->fd, pMsgBuff, MSG_SIZE, 0, NULL, NULL))
	{
		if (errno != EINTR)
		{
		 	perror("recvfrom");
